...
sName(game){const sugg=["株式会社テックブリッジ","合同会社ソルバー","株式会社クロスITソリューションズ","合同会社アクセルエンジニアリング"];const chips=sugg.map(s=>`<div class="name-chip" data-name="${s}">${s}</div>`).join("");
...
const avail=s.availableCases.map(c=>this.caseCard(c,s,false)).join("")||`<div class="empty-state"><div class="ei">📭</div><div class="et">今月の案件はありません</div></div>`;
...
<main class="center-panel"><div class="tab-bar"><button class="tab-btn active" id="tab-avail">案件ボード(${s.availableCases.length})</button><button class="tab-btn" id="tab-active">稼働中(${s.activeCases.length})</button></div><div class="tab-content" id="tab-content">${edu}<div class="section-title">今月の案件</div>${avail}</div></main>
...
const cn=document.getElementById("btn-ch-next");if(cn){document.querySelectorAll("[data-ch]").forEach(el=>{el.onclick=()=>{Sound.play("select");document.querySelectorAll("[data-ch]").forEach(e=>e.classList.remove("selected"));el.classList.add("selected");game.generateCandidates(el.dataset.ch);cn.disabled=false;};});cn.onclick=()=>{Sound.play("click");this.render(game);};}
...
bEvScreen(game){const s=game.state;document.querySelectorAll("[data-choice]").forEach(btn=>{btn.onclick=()=>{Sound.play("click");game.resolveEventChoice(parseInt(btn.dataset.choice));document.getElementById("app").innerHTML=`<div class="event-screen"><div class="event-card ev-neutral"><div class="ev-title">結果</div><div class="ev-outcome">→ ${s.eventOutcome}</div><button id="btn-after-event" class="btn btn-success" style="margin-top:16px;width:100%">来月へ →</button></div></div>`;document.getElementById("btn-after-event").onclick=()=>{Sound.play("next");game.nextMonth();this.render(game);};};});const af=document.getElementById("btn-after-event");if(af)af.onclick=()=>{Sound.play("next");game.nextMonth();this.render(game);};},
