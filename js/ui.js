...
<div class="cc-meta"><span>${"\u2605".repeat(cas.requiredSkill||cas.skill||1)}${"\u2606".repeat(5-(cas.requiredSkill||cas.skill||1))} \u30b9\u30ad\u30eb</span><span>${ri} ${cas.risk==="high"?"\u9ad8":cas.risk==="medium"?"\u4e2d":"\u4f4e"}</span><span>\u23f1 ${isActive?cas.monthsLeft+"\u30f6\u6708\u6b8b":cas.dur+"\u30f6\u6708"}</span>${(()=>{if(!isActive)return'';const elapsed=(cas.dur||0)-(cas.monthsLeft||0);const ec=elapsed>=12?'#e94560':elapsed>=6?'#f7971e':'#7c86a2';return elapsed>0?`<span style="color:${ec};background:${ec}18;border:1px solid ${ec}44;border-radius:3px;padding:1px 5px;font-size:10px">\u7a3c\u50cd${elapsed}\u30f6\u6708</span>`:''})()}</div>
${(cas.requiredTags&&cas.requiredTags.length>0)?`<div class="cc-tags">${cas.requiredTags.map(t=>`<span class="req-tag">${t}</span>`).join("")}</div>`:""}
${trustBadge}
${isActive&&eng?`<div class="cc-engineer">\ud83d\udc64 ${eng.name}(${eng.typeName}) <span style="font-size:10px;color:${(eng.stress||0)>=70?'#e94560':(eng.stress||0)>=50?'#f7971e':'#7c86a2'}">😤${eng.stress||0}%</span></div>`:""}
...
const cards=HIRING_CHANNELS.map(ch=>{
  ...
  const costStr=ch.cost===0?'✓ 無料':'¥'+Math.round(ch.cost/10000)+'万';
